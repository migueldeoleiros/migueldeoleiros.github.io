...
export const initHeroReveal = (): void => {
	...
	const canUsePointerEffect = !window.matchMedia('(pointer: coarse)').matches;
	...
	const updateRevealShape = () => {
		...
		for (let index = 0; index <= heroMotion.capSegments; index += 1) {
			...
			const x =
				headX + normalX * Math.cos(t) * radius + unitX * Math.sin(t) * radius;
			const y =
				headY + normalY * Math.cos(t) * radius + unitY * Math.sin(t) * radius;
			points.push(`${x} ${y}`);
		}
		...
		for (let index = 0; index <= heroMotion.capSegments; index += 1) {
			const t = (index / heroMotion.capSegments) * Math.PI;
			const x =
				tailAnchorX - normalX * Math.cos(t) * radius - unitX * Math.sin(t) * radius;
			const y =
				tailAnchorY - normalY * Math.cos(t) * radius - unitY * Math.sin(t) * radius;
			points.push(`${x} ${y}`);
		}
	...
	hero.addEventListener('mouseleave', (event) => {
		...
		hero.dataset.hover = 'true';
		const bounds = hero.getBoundingClientRect();
		const pointerExitDirection = getPointerExitDirection(event, bounds);
